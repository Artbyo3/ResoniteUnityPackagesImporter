...
### Uruki bounds experiment

MANUKA's avatar children had been deleted by the user; Uruki reproduces the reported issue. On `Uruki_poi`, moving the import parent 100 metres along X while compensating both child positions left the avatar visible in the fixed camera. All three local positions were restored and compared exactly with the saved originals. This argues against parent position alone causing the problem; it does not rule out a transform/bounds defect.
...
The importer default now uses `MediumPerBoneApproximate`. All 28 current `Uruki_poi` renderers were updated to that mode, with original values kept only in the ignored local evidence directory. The user subsequently reported that it no longer appears to disappear. This supports the workaround for this avatar, pending broader testing. Other avatar variants remain unchanged. The build has zero warnings/errors and 45 regressions pass; those tests do not validate visual culling. Parent backups, restored snapshots, and before/after camera captures were also kept only as local evidence.

### Bounds follow-up (2026-09-25)

The installed `E0DE7C450B166FA795FE99A8BDE1653BD8BD64259082EE78244DD3EF3FDD1334` build was confirmed in the live log. A fresh MANUKA import and a later clothing import completed with no missing-bone diagnostics and no skinned-bounds update exception. The user reports that the bounds change improved the disappearance problem by about 80%, but did not eliminate it. This rules out missing bone references for this test and leaves post-duplication bounds timing or approximation coverage as the leading causes.

The next build now keeps prefab, scene, and Modular Avatar renderers inactive for three world updates after hierarchy and bone changes, then reapplies the per-bone policy immediately before activation. This follows the engine's known safe delay for duplicated skinned hierarchies. It still needs a fresh in-game import before the remaining defect can be considered fixed.
